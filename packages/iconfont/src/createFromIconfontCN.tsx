...
export default function createFromIconfontCN<T extends string = string>(
  ...
  const Iconfont = React.forwardRef<HTMLSpanElement, IconFontProps<T>>((props, ref) => {
    ...
    if (props.type) {
      // Use the type directly as symbol id
      // iconfont scripts create symbols with id="icon-{name}" or id="{name}"
      // We use #icon-{type} to match the standard iconfont format
  ...
  return Iconfont as unknown as any;
}
