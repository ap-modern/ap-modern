...
import type { HTTPResponse } from './api-utils';
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export interface HTTPRequestInit<T = any> extends RequestInit {
  noAuthorize?: boolean;
  params?: T;
}
...
export const redirectToLogin = () => {
  ...
  if (typeof window !== 'undefined') {
    ...
    if (!window.location.pathname || window.location.pathname === '/') {
      ...
      navitagteUrl = `/login?callback_url=${window.location.pathname}`;
    }
...
export async function apiRequest<T>(
  ...
  return await makeRequest(endpoint, fetchOptions, responseClass, noAuthorize);
}
...
async function makeRequest<T>(
  ...
  const url = buildApiUrl(
    endpoint.startsWith('http') ? endpoint : `${defaultRequestOptions.baseURL}${endpoint}`,
    options.params
  );
  ...
  if (!response.ok) {
    ...
    if (response.status === 401 && !noAuthorize && !retryException) {
      ...
      if (defaultRequestOptions.fallback) {
        return defaultRequestOptions.fallback(response.status, () =>
          makeRequest(endpoint, options, responseClass, noAuthorize, () => {
